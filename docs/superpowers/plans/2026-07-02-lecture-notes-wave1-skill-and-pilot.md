...
- **Component styling uses Tailwind utility classes**, matching the codebase convention (see `src/components/RubricTable.astro`). Do NOT use `<style>` blocks. For theme-aware colors that must flip light/dark, use Tailwind arbitrary values referencing Starlight CSS variables, for example `text-[var(--sl-color-gray-3)]` or `border-[var(--sl-color-gray-5)]`.
