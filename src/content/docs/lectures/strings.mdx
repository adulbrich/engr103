...
```cpp title="Including the String Header"
#include <string>
```
...
```cpp title="String Comparison Example"
...
```cpp title="Hello World with Strings"
#include <iostream>
#include <string>
...
```cpp title="Reading an Integer with >>"
...
```plaintext frame="terminal" title="Output of leftover_data.cpp"
Number is: 3
...
```plaintext frame="terminal" title="User Input Example"
the quick brown fox jumps over the lazy dog
```

The `>>` operator stops at the first space, leaving the rest of the input in the buffer. The program will output:

```plaintext frame="terminal" title="Output of leftover_data_string.cpp"
...
The `std::getline()` function is part of the `<string>` header file. It takes two arguments:
...
```plaintext frame="terminal" title="User Input with std::getline()"
Enter a sequence of characters: the quick brown fox jumps over the lazy dog
...
To fix this, you can use `std::cin.ignore()` to discard the leftover characters (like the newline) from the input buffer before calling `std::getline()`.

```cpp title="Using std::cin.ignore()"
std::cin >> number;
std::cin.ignore(); // Discard the newline character left by >>
std::getline(std::cin, text);
```

`std::cin.ignore()` without arguments discards exactly one character in the input buffer, which is typically the newline left by the `>>` operator. This allows `std::getline()` to function as intended, reading the user's input without interference from leftover characters.

:::tip[Extra Safe `std::cin.ignore()`]
If you want to be extra safe (for example, if the user might type extra characters after the number), you can ignore *everything up to the next newline* instead of just one character:

```cpp title="Extra Safe std::cin.ignore()"
#include <limits>
...
The first argument is the number of characters to ignore, and the second argument is the delimiter at which to stop ignoring.
:::
