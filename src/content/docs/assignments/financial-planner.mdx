...
If the user selects an invalid option, display an error message and terminate the program imediately.
...
The output will be the required monthly payments needed to reach the future value goal, formatted as currency.

### Additional Details

Even though the formula is very flexible, make sure to validate inputs as described above, and handle errors or defaults gracefully. For example, what happens if the user inputs return rate of zero or if the user wants to save for 0 years?
