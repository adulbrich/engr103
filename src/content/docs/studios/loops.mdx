...
```
{/* IMPROVE: Mention that `rand()` generates the same sequence unless seeded with `srand(time(0))` (requires `<ctime>`), but for this exercise, deterministic behavior might be acceptable. */}
