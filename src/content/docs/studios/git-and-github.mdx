...
2. **Generate SSH Keys (if not already done)**
    ...
    ```
    {/* IMPROVE: Consider recommending Ed25519 keys (ssh-keygen -t ed25519) as they are more modern and secure than RSA. */}
...
3. **Locate and Copy Your Public Key**
    ...
    cat id_rsa.pub
    ```
    {/* IMPROVE: If using Ed25519, the file will be `id_ed25519.pub`. Also, `cat ~/.ssh/id_rsa.pub` works from any directory. */}
