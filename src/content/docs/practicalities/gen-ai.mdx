...
- **Hallucinations**: making up facts, library functions, or language rules.
- **Subtle bugs**: off-by-one errors, incorrect loop bounds, wrong types, missing imports.
...
You are a patient Python and Rust tutor for a first-year student.

Topic: [loops / functions / lists / strings / error handling]
...
- the **exact error message** the tool printed,
- the **smallest code snippet** that triggers it,
- what you expected vs what happened.

```text title="Explain this compiler error"
I am running Python 3 with python3 / compiling Rust with rustc.
...
This can be great for learning standard library functions, but verify with a trusted source: docs.python.org, doc.rust-lang.org, your notes, or the lecture material.

```text title="How do I use this?"
In Python, what does [input / list.append / int] do?
In Rust, what does [Vec::push / String::from / parse] do?
...
Show a minimal example, in Python and in Rust, of:
- reading a whole number from the console
- rejecting input that is not a number
- asking again until the input is valid

Keep each version under 30 lines and explain each part.
```
...
Please refactor this Python (or Rust) code to improve readability.
...
4. **Tricky use (hallucination bait)**
   - "What does `list.push()` do in Python?" (It does not exist; Python lists use `append`. See if the tool invents it.)
...
Every ENGR 103 assignment ends with a reflection in which you state plainly whether you used an AI tool and, if so, for what. Saying you did is never penalized; the disclosure is what is graded.
...
2. Does it run, and does the Rust version compile?
