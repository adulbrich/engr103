...
{/* TODO: improve page flow in particular for things that are out of scope */}
...
4.  **git**: a version control system to manage your code (we'll cover this in the [Git lecture](/lectures/git/)).
5.  **g++**: a compiler to turn your C++ code into a running program (we'll cover this in the [C++ Basics lecture](/lectures/cpp-basics/)).
...
```shell title="SSH Command to Access ENGR Servers"
...
```text title="ulbrical@flip4:~" frame="terminal"
===================================================================
...
```shell title="Simple Shell Prompt"
user@hostname:~$
```
...
```shell title="Basic Command Structure"
...
```shell title="Cat Command Example"
cat myfile.txt
``` 
...
```shell title="Copy Command Example"
...
```shell title="Tar Command Example"
...
```shell title="Print Working Directory Example Output"
...
```text frame="terminal" title="Path Example"
A/B/C.jpg
```
...
```text frame="terminal" title="Absolute Path Example"
...
```text frame="terminal" title="Relative Path Example"
A/B/C.jpg
```
...
```bash title="Make Directory Command"
mkdir <path>
```

For example

```bash title="Make Directory Example"
mkdir A
```
...
```bash title="Make Parent Directories Example"
mkdir -p A/B/C
```
...
```bash title="Clear Terminal Command"
clear
```
...
```bash title="Defining Environment Variable Example"
MY_VAR="Hello, World!"
echo $MY_VAR
```
...
```bash title="Command Substitution Example"
echo "Today is $(date)"
```
...
```bash title="Open File in vim Command"
vim <path>
```
...
```bash title="Create and Open hello.txt in vim"
vim hello.txt
```
...
```text title="vim hello.txt" frame="terminal"
█
~
~
~
...
```text title="vim insert mode" frame="terminal"
Hello, World!█
~
~
~
...
```text title="vim save and quit" frame="terminal"
Hello, World!
~
~
~
...
```text title=".vimrc" frame="terminal"
  1 set mouse=A
  2 set nu█
~
~
...
```bash title="List Directory Contents Command"
ls
```
...
```bash title="List All Files Command"
ls -a ~
```
...
```bash title="Cat Command Example"
cat hello.txt
```
...
```bash title="Concatenate Multiple Files Command"
...
- [Visual Studio Code](https://code.visualstudio.com/): A lightweight, extensible code editor. It is highly recommended if you want to try an IDE, especially for its **Remote - SSH** extension which works seamlessly with the ENGR servers.
- [CLion](https://www.jetbrains.com/clion/): A powerful, full-featured C++ IDE by JetBrains.
- [Zed](https://zed.dev/): A high-performance, multiplayer code editor.
- [Eclipse CDT](https://www.eclipse.org/cdt/): A classic open-source IDE.

:::caution[Self-Support Only]
**We do not provide support for setting up or troubleshooting IDEs.**

The teaching staff is trained to help you with the standard terminal-based environment (ssh, vim, git, g++). If you choose to use an IDE, you are responsible for its configuration and maintenance. Regardless of the tool you use, you must ensure your code compiles and runs correctly on the ENGR servers using `g++`.
:::
