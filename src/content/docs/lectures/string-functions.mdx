...
{/* TODO: proper class/object explanation, maybe on separate page */}
...
```cpp title="Accessing Object Members"
<object>.<member>
```
...
```cpp title="Using the .size() Function"
my_string.size();
```
...
```
Using `std::string::size_type` ensures your program can handle extremely large strings if necessary and aligns the syntax with the intent of the code.
...
```cpp title="Using the .at() Function"
std::cout << my_string.at(2) << std::endl;
```

:::note[Using `.at()` vs `[]`]
You may also see the **subscript operator** `[]` used to index strings:

```cpp title="Using the Subscript Operator"
std::cout << my_string[2] << std::endl;
```

This is common, but it does **not** perform bounds checking. If the index is invalid, `[]` can lead to undefined behavior. In contrast, `.at()` performs bounds checking and will throw an exception if the index is out of range. This will be discussed later when we cover arrays.
:::
...
```cpp title="Modifying a Character with .at()"
my_string.at(4) = 'E';
```
...
```plaintext frame="terminal" title="Output of Out-of-Bounds Access"
H
!
...
```plaintext frame="terminal" title="Output of string_functions.cpp"
