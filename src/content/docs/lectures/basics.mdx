...
- A *preprocessor directive* A *preprocessor directive* instructing the compiler to insert the contents of the `iostream` header file from the C++ standard library before compilation. 
...
We can modify the program , for example by adding another `std::out` statement:
