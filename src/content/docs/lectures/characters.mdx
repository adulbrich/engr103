...
```cpp title="Char Variable Declaration"
char myLetter = 'A';
```
...
Interestingly, the `char` data type is **also considered integral**. This is because, in binary computers, all data is stored as numbers. A variable's type determines how its binary representation is interpreted. Characters are stored as whole numbers in memory, and their corresponding character symbols are only displayed when interpreted by a terminal.

:::note[Char as an Integral Type]
A `char` stores a single byte (a pattern of bits). Whether `char` behaves like a signed or unsigned integer type is **implementation-defined** (it depends on the compiler/target). ASCII codes (see below) are non-negative, but the `char` type itself is not guaranteed to be non-negative.
:::
...
```cpp title="ASCII Conversion Example"
...
```cpp title="Character Literal Example"
std::cout << 'H';
std::cout << 'e';
std::cout << 'l';
std::cout << 'l';
...
```cpp title="Lowercase to Uppercase Conversion"
...
While these functions might work out-of-the-box, you should always include the appropriate header files (like `<cctype>`) to ensure compatibility across different compilers and environments.
