...
```plaintext frame="terminal" title="Compilation Error: Initialization of Out-of-Scope Variable"
...
2. **Implicit Initialization (Still Bad Practice)**: Global variables are **zero-initialized by default** in C++. If you forget to initialize them, your program may still run but produce incorrect results (a logic bug). Relying on this implicit initialization is still a bad habit.
