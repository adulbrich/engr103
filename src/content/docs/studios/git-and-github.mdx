...
3. **Locate and Copy Your Public Key**
    ...
    Navigate to the directory where your SSH keys are stored (default: `~/.ssh/`). By default, your SSH key files should be named `id_ed25519` and `id_ed25519.pub`. Use the following command to print the contents of the public key file (`id_ed25519.pub`):

    ```bash title="Display Public SSH Key"
    cat id_ed25519.pub
    ```

    The output should begin with `ssh-ed25519`. Highlight the contents and copy it to your clipboard.
...
Follow these steps to clone your Studio 2 GitHub repository to the ENGR servers. Before starting, review the [Git lecture notes](/lectures/git/).
...
4. **Clone the Repository**
    ...
    ```bash title="Clone Studio 2 Repository"
...
5. **Verify the Clone**
    ...
    ```bash title="Navigate to Cloned Repository"
    cd studio-2-XXX
    ```
