...
{/* IMPROVE: Explain that output buffers might not flush on crash, so using std::cerr or std::endl is important for debugging crashes. */}
...
{/* FIX: Verify if core dumps are actually disabled or redirected on the specific course infrastructure. */}
