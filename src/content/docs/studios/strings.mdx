...
{/* IMPROVE: Use `std::string::size_type` or `auto` instead of `int` for string positions to handle large strings correctly and match the return type of `find`. */}
...
{/* IMPROVE: Use `std::string::size_type` or `auto` for positions. */}
```cpp title="Solution 1"
int count_substr(std::string str, std::string substr) {
