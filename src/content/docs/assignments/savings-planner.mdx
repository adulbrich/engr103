...
- **Current amount**: Initial amount of money saved, which can be negative to represent debt, or current price of a specific item (USD).
...
{/* If rate is 0, it will fail if no error checking. */}

### Option 2: Recurring Savings Calculator
...
Ensure future value goal is greater than current savings. This rule applies even when current savings is negative.
...
- The required recurring savings needed to reach the future value goal, formatted as currency.
