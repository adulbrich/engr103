...
## Example: function call + parameters
...
Function calls (minimal teaching subset):

- Define `int`/`void` functions at top level (braces on their own lines)
- Run starts at `main()` (so include `int main()` and set `showWrapper={false}`)
- Pass arguments by value into parameters
- Return `int` values with `return expr;`

No pointers or heap yet.
