...
Discover the basics of Git, a powerful version control system. This guide covers configuring Git, creating and cloning repositories, staging and committing changes, and synchronizing with remote repositories like GitHub.
...
Suppose I've just cloned a repository containing a single file, `main.py`, with the following Python source code:
...
Now, suppose we modify `main.py` by adding an exclamation mark to the output string, as shown below:
...
Notice the line under `“Changes not staged for commit”`. Git lists `main.py` here, indicating that this file has been modified since the last commit.
...
Let's say we want to commit the changes made to `main.py` (with the exclamation mark added). First, we stage the file using:
...
![git status staged](./terminal-git-status-staged.png)

The output will now indicate that `main.py` is staged and ready to be included in the next commit. This step is crucial for ensuring the latest versions of files are saved properly in your repository.
...
For example, if I wanted to commit the changes to `main.py`, I might use:

```bash
git commit -m "Add exclamation mark to print statement"
```
...
Notice that there are no more modified or staged files—all changes have been committed. However, the output will say, `“Your branch is ahead of ‘origin/main’ by 1 commit.”` This means the new commit exists in your local repository but hasn't been synced with the remote GitHub repository yet. If you visit your GitHub repo at this point, it will still show the outdated version of `main.py` without the exclamation mark.
...
Once you run `git push`, executing `git status` again will confirm that the local and remote repositories are now synchronized. If you refresh your GitHub repository page, you'll see that `main.py` has been updated, and the exclamation mark is present.
...
In this case, there are three commits. The first one (with the message “Initial commit”) was created when the repository was initialized and the second one when the first version of `main.py` was added. The third one reflects the recent addition of the exclamation mark.
...
For example, if I modify `main.py` again to add another exclamation mark and then run:

```bash
git diff main.py
``` 

The output will highlight changes, showing removed lines in red (prefixed with a `-`) and added lines in green (prefixed with a `+`). In this case, the only change would be the addition of another exclamation mark.

![git diff](./terminal-git-diff.png)

If I stage and commit the changes to `main.py`, running `git log` will now show four commits.
