...
```cpp title="If Statement Structure"
if (condition_A) {
...
```cpp title="If Statement Without else"
int main() 
{
    // Prompt user for a number
    int number = prompt_for_integer();
...
```cpp title="If Statement with Single Condition"
int main() 
{
    // Prompt user for a number
    int number = prompt_for_integer();
...
```cpp title="Mutually Exclusive if-else Statement"
int main() 
{
    // Prompt user for a number
    int number = prompt_for_integer();
...
```cpp title="Changing if-if to if-else if"
/*
 * Function: print_pass_or_fail
 * Description: Prints whether the user passed or failed based on the grade percentage
 * Parameters:
...
```cpp title="Changing else if to else"
/*
 * Function: print_pass_or_fail
 * Description: Prints whether the user passed or failed based on the grade percentage
 * Parameters:
...
```cpp title="Implied Curly Braces Example"
if (2 + 2 == 4)
...
```cpp title="Misleading Indentation Example"
if (2 + 2 == 100)
...
```cpp title="Corrected Indentation Example"
if (2 + 2 == 100)
    std::cout << "Woo! Two plus two is 100!" << std::endl;
...
```cpp title="Using Curly Braces Example"
if (2 + 2 == 100) {
    std::cout << "Woo! Two plus two is 100!" << std::endl;
}
...
```cpp title="Nested If Statements Example"
if (i_like_spaghetti) {
    if (i_like_broccoli) {
...
```cpp title="Nested If Statements Behavior"
if (i_like_spaghetti) {
    if (i_like_broccoli) {
        std::cout << "I like both spaghetti AND broccoli!" << std::endl;
    } else {
...
```cpp title="Using Logical AND Instead of Nested Ifs"
...
```cpp title="Using Logical AND with else if"
if (i_like_spaghetti && i_like_broccoli) {
    std::cout << "I like both spaghetti AND broccoli!" << std::endl;
...
```cpp title="Misleading Nested If Example"
if (2 + 2 == 4)
    if (4 + 5 == 8)
        std::cout << "A" << std::endl;
else
...
```cpp title="Corrected Nested If Indentation Example"
if (2 + 2 == 4)
    if (4 + 5 == 8)
        std::cout << "A" << std::endl;
    else
...
```cpp title="Using Curly Braces in Nested If Statements"
if (2 + 2 == 4) {
    if (4 + 5 == 8) {
        std::cout << "A" << std::endl;
    } else {
...
- The `&` and `|` operators are **bitwise** operators. When used with `bool`, they can behave like an AND/OR that *does not* short circuit.
- Similarly, `||` is the logical OR operator *with* short circuiting, while `|` will evaluate both operands.
...
-	A key difference is that `&&` and `||` are **logical** operators, while `&` and `|` are **bitwise** operators. Prefer `&&` and `||` for boolean logic unless you have a very specific reason.
...
(1 > 100) && (std::sqrt(std::pow(2, 5)) * 3.141592 + std::pow(2.71, 9.81) > std::abs(-std::sqrt(100)))
```
...
```cpp title="Incomplete Logical Operand Example"
x == 2 || 0
```
...
```cpp title="Implicit Conversion Example"
x == 2 || false
```
...
```cpp title="Simplified Expression"
x == 2
```
...
```cpp title="Correct Logical Operand Example"
x == 2 || x == 0
```
...
```cpp title="Risky Chained Inequality Example"
1 <= x <= 10
```
...
```cpp title="Risky Chained Inequality Breakdown"
(1 <= x) <= 10
```
...
```cpp title="Correct Chained Inequality Example"
1 <= x && x <= 10
```
...
```cpp title="Alternative Correct Chained Inequality Examples"
x >= 1 && x <= 10
x <= 10 && x >= 1
```
...
```plaintext frame="terminal" title="Compiler Error: Single Else Without If"
...
```cpp title="Yoda Notation Example"
if (5 == x)
```

Instead of:

```cpp title="Standard Notation Example"
if (x == 5)
```
...
Modern compilers will often warn you about assignment in a condition if you enable warnings (for example, `g++ -Wall` may emit a warning like "using the result of an assignment as a condition without parentheses"). Enabling warnings is a great way to catch this error without relying on Yoda notation.
...
```cpp title="Floating Point Equality Example"
...
```cpp title="Floating Point Approximate Equality Example"
if (std::abs(some_floating_point_value - some_other_floating_point_value) < 0.000001) {
    // Code here
}
```
