...
Review the [text editors](../../lectures/development-environment/#text-editors-vim-nano) section of the Development Environment lecture notes. Use vim to write the following code to the file:
...
IT supports also provides some directions to configure SSH keys. You can find them [here](https://it.engineering.oregonstate.edu/ssh-keygen).