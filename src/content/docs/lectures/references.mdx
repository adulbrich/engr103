...
```cpp title="Reference Variable Declaration Syntax"
<type>& <name>;
```
...
```cpp title="Integer Reference Declaration Example"
...
```cpp title="Alternative Reference Variable Declaration Syntax"
<type> &<name>;
```
...
```cpp title="Function with Reference Parameter"
void change_value_and_print(int& x) 
{
    ...
}
...
```cpp title="Reference Assignment Example"
int a = 10;
...
Here is a small interactive example that shows that aliasing behavior in the teaching-model memory stepper:
...
```cpp title="Chained References Example"
std::string x;
std::string& y = x;
std::string& z = y;
...
```cpp title="Invalid Reference to Literal"
...
```cpp title="Function with Const Reference Parameter"
#include <iostream>
...
```cpp title="Function with Const Array Parameter"
