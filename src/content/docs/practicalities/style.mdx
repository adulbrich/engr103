...
We also recommend the One True Brace Style for braces. For function declaration, the opening and closing braces are on their own line, aligned vertically. Inside a function, a statement block's opening brace is on the same line as the control statement, and the closing brace is on its own line, aligned vertically with the control statement.
...
```
Alternatively, you could use `pow(radius, 3)` from the `<cmath>` library. Note that direct multiplication is often more efficient for small integer powers.
...
```c++ title="Function-Level Comment"
/**
...
Lastly, **avoid daisy-chaining functions**. Daisy-chaining happens when responsibilities are divided into functions based primarily on procedural order rather than logical semantics. This results in a “deep” function call graph, where each function performs one step and calls the next function, and so on, until the final function is reached.

```c++
// bad_daisy_chaining.cpp
#include <iostream>
#include <string>
...
{
    std::string out = std::string("Result: ") + std::to_string(value);
    writeResult(out);
}
...
int normalizeNegativeToZero(int x) 
{
    int n = x < 0 ? 0 : x;
...
{
    int v = std::stoi(s);
    return normalizeNegativeToZero(v);
}
...
```mermaid
flowchart TB
  Main[main] -->|calls| A[readInput]
  A -->|calls| B[stringToInteger]
  B --> C[normalizeNegativeToZero]
  C --> D[multiplyByTwo]
  D --> E[formatOutput]
  E --> F[writeResult]
```

Instead, aim to **separate responsibilities semantically**, not procedurally.
...
```c++
// good_semantic_decomposition.cpp
#include <iostream>
#include <string>
...
void processRequest() 
{
    ...
    int parsed = stringToInteger(token);
    int normalized = normalizeNegativeToZero(parsed);
    int computed = multiplyByTwo(normalized);

    std::cout << formatResult(computed) << '\n';
}
...
```mermaid
flowchart LR
  subgraph LowLevel [Low level building blocks]
    P[stringToInteger]
    N[normalizeNegativeToZero]
    C[multiplyByTwo]
    F[formatResult]
  end
...
Use the prompt below to guide AI code generation according to this style guide.

```md title="AI Instruction Prompt for C++ Styling"
Rewrite or reformat this C++ code according to the following rules:

- Use spaces (not tabs) with consistent 2- or 4-space indentation.
- Use One True Brace Style: `{` on a separate line for functions,  `{` on the same line inside functions, `}` aligned on its own line.
- Add whitespace around operators and after commas; separate logical sections with blank lines.
- Keep lines under 80 characters; wrap long expressions at operators; split long strings.
- Use camelCase or snake_case for variables/functions and PascalCase for classes; all names must be descriptive.
- Add a file-level header comment and function-level block comments for complex functions; keep inline comments minimal and meaningful.
- Do not use global variables except for constants.
- Ensure each function has a single responsibility, is reasonably short, and avoids unnecessary side effects.
- Avoid daisy-chained functions; prefer semantic decomposition.
- Remove repeated code and simplify conditionals where possible.
- Do not “over-compress” code; prioritize readability and maintainability.
```
