...
```cpp title="Do-While Loop Syntax"
do {
    <loop body>
} while (<condition>);
```
...
```cpp title="Do-While Loop with Assignment in Condition"
do {
    double number = prompt_for_number();
    running_sum += number;
...
```cpp title="Do-While Loop without Condition Variable"
do {
    double number = prompt_for_number();
    running_sum += number;
...
```cpp title="Count-Based While Loop Example"
#include <iostream>
...
```cpp title="Count-Based For Loop Example"
#include <iostream>
...
```cpp title="For Loop Scope Example"
#include <iostream>
...
```cpp title="For Loop with External Counter"
#include <iostream>
...
```plaintext frame="terminal" title="Compilation Error: Shadowing in For Loop"
...
You can only shadow it by introducing an additional nested block `{}`, but this is confusing and should be avoided. Use distinct names instead.
:::
...
- Condition: Comparing the counter to the target value using a less-than operator `<` or greater-than operator `>`.
...
```cpp title="Common For Loop Pattern"
for (int i = 0; i < n; i++) {
    ...
}
```
...
```cpp title="For Loop with Offset"
for (int i = 0; i < 48; i++) {
    if (i > 0) {
        std::cout << ", ";
    }
...
```cpp title="For Loop over Desired Range"
for (int i = 7; i <= 54; i++) {
    if (i > 7) {
        std::cout << ", ";
    }
...
```cpp title="For Loop with Decrementing Counter"
...
```plaintext frame="terminal" title="Multiplication Table Output"
