...
Two rotation rules across the term. First, **vary the trio from activity to activity**: if every activity picks the same three types in the same order, students stop attempting and start pattern-matching the page. Second, **peer instruction must appear regularly, not once**: the exams are multiple choice, so a concept question with misconception distractors is the exams' most direct rehearsal, and it belongs in the rotation from the early weeks on. A predict-then-run or peer-instruction prompt may also take **complete-the-code form** (a short program with one labeled blank, and either a free prediction or candidate lines to choose from); use that form at least occasionally, since it is one of the published exam archetypes.
...
4. **Three `##` sections**, separated by `---`, one per chosen block type. Give each a short **descriptive heading that says what the student does**, for example `## Predict the Value`, `## The Same Division, Two Languages`, or `## Find the Silent Bug`. Never title a section `Block 1`, `Block 2`, or any other generic label: the heading names the task, not its slot in the session. Each section opens with 1 to 2 sentences of framing (what problem it poses, and why it matters, practically, not conceptually), then the prompt, then the `<Reveal>` holding the resolution. **The framing must never pre-teach the rule or name the trap the prompt tests.** A preamble that says "remember that Python's `/` produces a float while Rust's truncates" directly above a prediction whose whole point is that difference has already closed the gap the block exists to open; the student predicts, the reveal explains. If a sentence in the framing would help the student answer, move it into the reveal. Each block must also fit its 13-minute slot: one prompt, one reveal; a block that stacks two full programs is two blocks.
...
There is no penultimate artifact section and no closing "further reading" section on an activity page: those are cs312 tutorial conventions built around a build-something walkthrough, and an ENGR 103 activity is a bare-concept session graded for completeness (students submit their Python or Rust files on Canvas), not a build-an-artifact exercise. The completeness grading is a Canvas-side detail; individual activity pages still carry no grading or submission boilerplate (that lives in the activities introduction page).
...
- No framing that pre-teaches the rule or names the trap its own prompt tests; the reveal owns the explanation.
- No block that overruns its 13-minute slot (two full programs in one block is two blocks), and no fourth rotating block.
