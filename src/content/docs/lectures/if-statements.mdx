...
However, some types of logic cannot be implemented with purely sequential programs. For example, imagine a program that behaves as follows: if the user inputs “1,” the program outputs “good,” but if the user inputs “0,” it outputs “bad.” This is an example of conditional logic--logic that requires specific actions to be taken only under certain conditions that are evaluated during runtime.

Since conditional logic cannot be expressed in a purely sequential program, we need to explore how to implement such logic in C++.

## Booleans

To write a program that performs certain operations when a condition is met, we need a way to represent the satisfaction of that condition in our code.
