...
```cpp title="Declaring an Array of Strings"
...
```plaintext frame="terminal" title="Array Base Address Example"
0x16b82af88
```

This hexadecimal value represents the array's base address.

:::note[Array Decay and Character Arrays]
You might wonder why `std::cout` prints a memory address for the array above. This happens because of a mechanism called **decay**.

"Decay" describes how an array loses its type information (like its size) and converts into a pointer to its first element (e.g., `&my_list_of_numbers[0]`) when used in most expressions. So, `std::cout` receives a memory address and prints it.

However, `char` arrays behave differently with `std::cout`. Even though they also decay to a pointer (`char*`), `std::cout` has a special rule for `char*`: it assumes the pointer points to a **C-string** (text) and prints characters until it finds a null terminator (`\0`).

```cpp title="Special Case for char Arrays"
char name[5] = {'B', 'o', 'b', '\0'};
std::cout << name << std::endl; // Prints "Bob", not an address
```

If your `char` array is missing the `\0` terminator, `std::cout` will keep reading past the end of the array until it finds a zero byte in memory, often printing garbage characters or crashing.
...
```cpp title="Syntax for Accessing an Array Element"
<array name>[<index>]
```
...
```cpp title="Declaring and Initializing an Array"
...
```cpp title="Printing an Array Element"
...
```cpp title="Reading Integers Into an Array"
...
```cpp title="Reading Strings Into an Array Using std::getline"
...
```cpp title="Invalid Array Assignment"
int numbers[100];
...
```cpp title="Copying Array Elements Manually"
for (int i = 0; i < 100; i++) {
...
```cpp title="Array Declaration with Initializer List"
int arr[5] = {1, 2, 3, 4, 5};
```
...
```cpp title="Invalid Array Initialization after Declaration"
int arr[5];
...
An `lvalue` is said to be non-modifiable if it cannot be assigned a new value. For example, a constant variable is an `lvalue`, but it is non-modifiable because you can't assign a new value to it.
...
```cpp title="Partially Initializing an Array"
int arr[5] = {1, 2};
```
...
```cpp title="Function to Print an Array of 10 Strings"
...
```cpp title="Calling print_strings with an Array"
int main() 
{
...
It is important to note that the `[10]` in the function parameter `void print_strings(std::string my_strings[10])` is **ignored by the compiler**. In C++, array parameters decay to pointers. This means the function will accept an array of *any* size, not just 10.

This can be dangerous. If you pass an array of size 5 to this function, the code will compile, but the loop inside `print_strings` will still try to access indices 0 through 9. Accessing indices 5 through 9 results in undefined behavior (a buffer overflow).

```cpp
// array_parameter_decay.cpp
int main() 
{
    std::string small_list[3] = {"one", "two", "three"};

    // This COMPILES, but causes a buffer overflow at runtime!
    print_strings(small_list); 
    return 0;
}
```
To write a function that safely handles arrays of different sizes, you should omit the size in the brackets (`[]`) and pass the size as a separate argument. Note that the `[]` syntax is valid only for function parameters, not for local variable declarations.

```cpp title="Function with Flexible Array Size"
...
```cpp title="Calling print_strings with Size Parameter"
int main() 
{
    std::string array_of_strings[10];
...
entirely separate memory and separate data. In such a case, modifying the parameter's data will not affect the argument's data. 

```cpp title="Passing Non-Array Variable by Value"
void foo(int x) 
{
    x += 1;
}
