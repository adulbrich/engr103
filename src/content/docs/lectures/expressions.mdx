---
title: "Expressions"
description: "Understand expressions in C++ and how they represent data. This guide covers types, values, memory allocation, primitive data types, and the binary system, providing a solid foundation for programming."
sidebar:
  order: 5
---
...
import MemoryPoolDiagram from '../../../components/MemoryPoolDiagram.astro'
...
If we consider the whole `std::cout << "Hello, World!" << std::endl;` expression, including the `<<` operators, it is of type `std::ostream&` (a reference to an output stream) and the value is the stream object itself. This is outside the scope of this class.
:::
...
Expressions have types and values, which must be stored internally by the computer. Modern computers use the binary numeric system for data representation.
...
Think of storage or memory as a large collection of *boxes*, each with a **unique address**. Each box can hold a **fixed** amount of data (typically one byte). When a program runs, it requests boxes from memory to store data temporarily while executing instructions.
...
- `bool`: Usually requires 1 byte, but on some older systems, it may occupy more space (e.g., 2 or 4 bytes).

<MemoryPoolDiagram />

:::note[Typically?]
The term "typically" is used because the C++ standard does not mandate exact sizes for most primitive data types, except for `char` and `bool` (both are 1 byte). Instead, it specifies minimum ranges that these types must support, allowing some flexibility in their implementation. The actual size can vary depending on the system architecture and compiler implementation.
:::
