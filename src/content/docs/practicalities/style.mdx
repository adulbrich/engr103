...
We also recommend the One True Brace Style for brackets. For function declaration, the opening and closing brackets are on their own line, aligned vertically. Inside a function, a statement block's opening bracket is on the same line as the control statement, and the closing bracket is on its own line, aligned vertically with the control statement.
...
- Use K&R brace style: `{` on the same line, `}` aligned on its own line.
